...
export default class GameController {
  ...
  init() {
    ...
    this.gamePlay.drawUi(Object.keys(themes)[this.gameState.level - 1]);
  ...
  onCellClick(index) {
    ...
    // Если ход не возможен, сообщение об ошибке
    ...
    // Если ячейка пустая при клике на неё return
    if (!this.getChar(index)) {
      return;
    };

    // Если клик на бота, сообщение об ошибке
    if (this.getChar(index) && this.isBotChar(index) && !this.isAttack(index)) {
      GamePlay.showError('Это не ваш персонаж');
    };

    // Если клик на игрока, выделяем клетку желтым
  ...
  onCellEnter(index) {
    ...
    // Если в ячейке игрок, при наведении на ячейку курсор = pointer
    if (this.getChar(index) && this.isUserChar(index)) {
      this.gamePlay.setCursor(cursors.pointer);
    };
    // Если перемещение в клетку с курсором возможно, при наведении выделяем ячейку зелёным
    if (this.gameState.selected !== null && !this.getChar(index) && this.isMoving(index)) {
      this.gamePlay.setCursor(cursors.pointer);
      this.gamePlay.selectCell(index, 'green');
    };
    // При наведении на персонажа показать инфо
    if (this.getChar(index)) {
      const char = this.getChar(index).character;
      const message = `\u{1F396}${char.level}\u{2694}${char.attack}\u{1F6E1}${char.defense}\u{2764}${char.health}`;
      this.gamePlay.showCellTooltip(message, index);
    };
    // Если атака возможна, при наведении выделить ячейку красным
    ...
    // Если атака или перемещение не возможны, при наведении курсор = notallowed
  ...
  getBotsResponse() {
    ...
    if (target) {
      const damage = Math.max(
        bot.character.attack - target.character.defense, bot.character.attack * 0.1
      );
  ...
  getGameResult() {
    ...
    if (this.botTeam.members.size === 0 && this.gameState.level === 4) {
      this.scoringPoints();
      this.gameState.statistics.push(this.gameState.points);
      GamePlay.showMessage(`Поздравляем! Победа! Количество набранных очков: ${this.gameState.points},
    ...
    if (this.botTeam.members.size === 0 && this.gameState.level <= 3) {
      ...
      GamePlay.showMessage(
        `Уровень пройден ${this.gameState.level} Количество набранных очков: ${this.gameState.points}`
      );
      this.gameState.level++;
      this.getLevelUp();
    };
  ...
  levelUp(team) {
    ...
    function paramsUp(person) {
      person.attack = Math.floor(Math.max(person.attack, person.attack * (0.8 + person.health / 100)));
      person.defense = Math.floor(Math.max(person.defense, person.defense * (0.8 + person.health / 100)));
      person.health = (person.health + 80) < 100 ? person.health + 80 : 100;
    };
